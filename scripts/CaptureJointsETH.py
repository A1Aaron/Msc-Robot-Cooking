...
def main():  
    ...
    rospy.loginfo("[INFO] Starting the CaptureJointETH node")
    rospy.init_node('CaptureJointETH',anonymous=True)
    ...
    while capL.isOpened():
        ...
        def callback(joints):
            ...
            if np.size(joints.data) == 7:
                # FOR HOME COBOT
                # jointlist = (joints.data[0],joints.data[1],joints.data[2],joints.data[3],joints.data[4],joints.data[5])
            
                # FOR UNI COBOT
                jointlist = (-joints.data[0],-joints.data[1],joints.data[2],-joints.data[3],-joints.data[4],-joints.data[5])
                
                # Used for auto capture
                if np.size(jointlist) and np.size(Tposelist) > 1 and time.time()-init_time >2:
                    jointarray.append(jointlist)
                ...
                if keys== ord('q'):
                    ...
                    rospy.loginfo("Saving Joint Array: {}".format(jointarray))
                    np.save('jointarray',jointarray)
                    ...
                    if np.size(jointlist) and np.size(Tposelist) > 0:
                        jointarray.append(jointlist)
                        Tposearray.append(Tposelist)
                        Rposearray.append(Rposelist)
            ...
            rospy.loginfo("Joint states heard were: [{}]".format (joints))                
